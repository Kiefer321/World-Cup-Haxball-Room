...
const headlessTokenURL = 'https://www.haxball.com/headlesstoken';

// Room configuration
const roomName = 'COPA DO MUNDO COM BOT | SELEÇÕES COM UNIFORMES';

// Game mode configuration
const TEAM_SIZE = 3;
const STARTING_MATCHES_COUNT = 4;
const MINIMUM_PLAYERS = TEAM_SIZE * STARTING_MATCHES_COUNT * 2;

const CAPTAIN_VOTING_TIME = 10;
const TEAM_CONFIG_TIME = 15;
const MATCH_INTERMISSION = 3;
...
const requestTokenFromUser = () => {
    const newToken = readline.question(`Por favor, insira outro token obtido no site : ${headlessTokenURL}\n`);
    
    fs.writeFileSync('.env', `HAXBALL_TOKEN=${newToken}\n`, { flags: 'w' });
...
const startRoom = async () => {
    let token = process.env.HAXBALL_TOKEN;

    if (!token) {
        console.log('Nenhum token foi fornecido');
        token = requestTokenFromUser();
    }
